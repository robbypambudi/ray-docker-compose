...
def build_app(cli_args: Dict[str, str]) -> serve.Application:
    """Builds the Serve app to utilize all available resources."""

    # Determine the accelerator type
    accelerator = cli_args.pop("accelerator", "GPU")

    # Parse vLLM arguments
    parsed_args = parse_vllm_args(cli_args)
    engine_args = AsyncEngineArgs.from_cli_args(parsed_args)
    engine_args.worker_use_ray = True

    # Set tensor parallel size to the number of GPUs available
    tp = engine_args.tensor_parallel_size or 2  # Default to 2 if not set
    ...
    # Allocate resources for vLLM actors
    for _ in range(num_actors):
        pg_resources.append({"CPU": num_cpus_per_actor, accelerator: 1})

    # Use STRICT_PACK strategy to colocate actors on the same node
